...
const GameCard: React.FC<{ game: EducationalGame; onClick?: () => void }> = React.memo(({ game, onClick }) => {
    const modeStyle = {
    ...
    return (
        <Wrapper 
            onClick={onClick}
            className={`w-full text-left bg-white p-4 rounded-lg shadow-sm border border-gray-200 transition-all duration-200 ${onClick ? 'cursor-pointer hover:ring-2 hover:ring-orange-300 hover:shadow-md' : ''}`}
        >
            ...
            <p className="text-sm text-gray-700 mt-2 line-clamp-2"><strong>How to Play:</strong> {game.howToPlay}</p>
            <p className="text-sm text-gray-700 mt-2 line-clamp-2"><strong>Learning Goal:</strong> {game.learningGoal}</p>
            <div className="mt-3 flex justify-between items-center">
                <span className={`px-2 py-1 text-xs font-semibold rounded-full ${modeStyle[game.mode]}`}>{game.mode} Activity</span>
                {onClick && (
                     <div className="px-3 py-1.5 text-xs font-semibold text-white bg-green-500 rounded-full flex items-center space-x-1 hover:bg-green-600 transition-colors duration-200">
...
const LevelAccordion: React.FC<{ level: string; games: EducationalGame[]; defaultOpen?: boolean; navigateTo: (view: string, title: string, props?: any) => void; }> = React.memo(({ level, games, defaultOpen = false, navigateTo }) => {
    const [isOpen, setIsOpen] = useState(defaultOpen);
    
    // Memoize the game list to prevent unnecessary re-renders
    const gameList = useMemo(() => games.map((game, index) => {
        const isPlayable = game.gameName === 'Math Sprint';
        return (
            <GameCard 
                key={`${level}-${game.gameName}-${index}`} 
                game={game} 
                onClick={isPlayable ? () => navigateTo('mathSprintLobby', 'Math Sprint', {}) : undefined}
            />
        )
    }), [games, level, navigateTo]);
...
const FeaturedGameCard: React.FC<{ title: string; description: string; icon: React.ReactNode; bgColor: string; onClick: () => void; }> = React.memo(({ title, description, icon, bgColor, onClick }) => (
    <div className={`flex-shrink-0 w-60 md:w-64 h-44 md:h-48 ${bgColor} rounded-2xl p-4 flex flex-col justify-between text-white shadow-lg transition-all duration-300 hover:shadow-xl hover:scale-105`}>
        <div>
        ...
        <button onClick={onClick} className="self-start bg-white/20 px-4 py-1.5 rounded-full text-xs font-bold hover:bg-white/40 transition-all duration-300 transform hover:scale-105">
            Play Now
        </button>
    </div>
));
...
const GamesHubScreen: React.FC<GamesHubScreenProps> = ({ navigateTo }) => {
    ...
    // Memoize featured games to prevent unnecessary re-renders
    ...
    // Memoize the level accordions to prevent unnecessary re-renders
    const levelAccordions = useMemo(() => {
        return levels.map((level) => {
            if (!gamesByLevel[level]) return null;
            return (
                <LevelAccordion 
                    key={level} 
                    level={level} 
    ...
    }, [gamesByLevel, levels, studentLevel, navigateTo]);
