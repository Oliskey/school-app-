...
import { mockAdminConversations } from '../../data';
...
const AdminMessagesScreen: React.FC<AdminMessagesScreenProps> = ({ navigateTo }) => {
    const [searchTerm, setSearchTerm] = useState('');
    const theme = THEME_CONFIG[DashboardType.Admin];

    const filteredConversations = useMemo(() => {
        return mockAdminConversations
    ...
    return (
        <div className="flex flex-col h-full bg-gray-50 relative">
                    ...
                    {filteredConversations.map(convo => (
                        <li key={convo.id}>
                            <button
                                ...
                                className="w-full text-left p-4 flex items-center space-x-4 hover:bg-indigo-50/50 transition-colors"
                            >
            ...
            <div className="absolute bottom-6 right-6">
                <button
                    onClick={() => navigateTo('adminNewChat', 'New Chat')}
