...
// Helper function for grading based on the user-provided scale
...
const AdminStudentReportCardScreen: React.FC<AdminStudentReportCardScreenProps> = ({ student }) => {
    ...
    const allSubjects = useMemo(() => {
        ...
        Object.values(reportsByTerm).forEach(report => {
            if (report) {
    ...
    if (Object.values(reportsByTerm).every(r => !r)) {
      return (
            ...
            <div className="max-w-4xl mx-auto">
                ...
                <div className="printable-area bg-white p-4 sm:p-6 rounded-lg border border-gray-200 shadow-sm">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 text-gray-900 font-sans">
                        ...
                        <InfoField label="Session" value={Object.values(reportsByTerm).find(r => r)?.session || 'N/A'} />
                    ...
                    <div className="overflow-x-auto text-sm">
                                ...
                                {allSubjects.map(subject => {
                                    ...
                                    return (
                                        <tr key={subject}>
                                            ...
                                            {terms.map(term => {
                                                const report = reportsByTerm[term];
                                                const record = report ? report.academicRecords.find(r => r.subject === subject) : null;
                                                const cellContent = record ? record.total : (report === undefined ? 'N/E' : (report.isPublished ? '-' : <span className="text-amber-600 font-bold">Draft</span>));
                                                return <td key={term} className="p-2 border border-gray-300 text-center">{cellContent}</td>;
                                            })}
                        ...
                        <p className="text-xs text-gray-500 mt-1 font-sans">N/E: Not Entered</p>
                    </div>

                    <SectionHeader title="Overall Performance (Based on Published Terms)" />
